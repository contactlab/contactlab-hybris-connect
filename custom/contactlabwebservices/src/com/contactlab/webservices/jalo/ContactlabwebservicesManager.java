 ...
 *  
 */
package com.contactlab.webservices.jalo;
...
/**
 * This is the extension manager of the Contactlabwebservices extension.
 */
public class ContactlabwebservicesManager extends GeneratedContactlabwebservicesManager {
    /** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
    private static final Logger LOG = Logger.getLogger(ContactlabwebservicesManager.class.getName());
     ...
     */
    public static ContactlabwebservicesManager getInstance() {
        return (ContactlabwebservicesManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager()
                .getExtension(ContactlabwebservicesConstants.EXTENSIONNAME);
    }
     ...
     */
    public ContactlabwebservicesManager() // NOPMD
    {
        if (LOG.isDebugEnabled()) {
            LOG.debug("constructor of ContactlabwebservicesManager called.");
        }
    }
    ...
    public void init() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("init() of ContactlabwebservicesManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("destroy() of ContactlabwebservicesManager called, current tenant: " + getTenant().getTenantID());
        }
    }
